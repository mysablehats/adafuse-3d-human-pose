...
class RealData(JointsDataset):
    ...
    def __getitem__(self, idx):
        ...
        for item in items:
            ...
            ##I think i is the image, lets test this out
            #print(i)
            #print(dir(i))
            #print(type(i))
            #ii = T.ToPILImage()(i)
            ...
            #ii.show()
            #image = i.cpu().numpy()
            #image = np.transpose(i.cpu().numpy(), (2,1,0))
            #image = np.transpose(i.cpu().numpy(), (1,2,0))
            #image = np.transpose(i.cpu().numpy(), (1,2,0))
            #image = i.permute(1,2,0).cpu().numpy()
            #iimage = cv2.absdiff(image, 255)
            #cv2.imshow("iiii", image)
            #cv2.waitKey(0)
