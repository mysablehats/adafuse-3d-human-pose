...
def main():
    ...
    ## to understand this spaget we need to break this eval
    print("Eval nonsense:%s"%('dataset.' + config.DATASET.TEST_DATASET))
    #valid_dataset = eval('dataset.' + config.DATASET.TEST_DATASET)(
    ## in our case this is just dataset.unrealcv
    valid_dataset = dataset.realdata(
        ...
        ## torch.utils.data.DataLoader allows for an iterable dataset, so here we can implement an __iter__() function which will get the latest picture(s)
    ...
    perf_indicator = predict(config, valid_loader, valid_dataset, model, criterion,
